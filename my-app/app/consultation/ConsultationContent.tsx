...
function StepReview({ form, onSubmit, onBack, submitting, error }: { form: FormState; onSubmit: () => void; onBack: () => void; submitting: boolean; error: string }) {
  ...
  return (
    <div>
      ...
      <div className="border border-stone-100 mb-10 max-w-md">{rows.map((row, i) => <div key={i} className={`flex gap-6 px-6 py-4 ${i !== rows.length - 1 ? "border-b border-stone-50" : ""}`}><span className="text-[10px] tracking-[0.2em] uppercase text-stone-400 w-20 flex-shrink-0 font-light" style={{ fontFamily: "'DM Sans', sans-serif" }}>{row.label}</span><span className="text-[13px] text-stone-700 font-light leading-relaxed" style={{ fontFamily: "'DM Sans', sans-serif" }}>{row.value}</span></div>)}</div>
      {error && <div className="mb-6 border border-red-100 bg-red-50 px-5 py-4 text-[12px] text-red-600 font-light" style={{ fontFamily: "'DM Sans', sans-serif" }}>{error}</div>}
      <div className="flex items-center gap-4">
        <button onClick={onBack} disabled={submitting} className="inline-flex items-center gap-2 text-[12px] tracking-[0.15em] uppercase text-stone-400 hover:text-stone-700 transition-colors font-light disabled:opacity-40" style={{ fontFamily: "'DM Sans', sans-serif" }}><ArrowLeft size={13} strokeWidth={1.5} /> Back</button>
        <button onClick={onSubmit} disabled={submitting} className="group inline-flex items-center gap-3 px-8 py-4 bg-stone-800 text-[#FAF8F5] text-[12px] tracking-[0.2em] uppercase font-light hover:bg-stone-700 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed" style={{ fontFamily: "'DM Sans', sans-serif" }}>{submitting ? "Confirming..." : "Confirm Appointment"}<Check size={14} strokeWidth={1.5} className="group-hover:scale-110 transition-transform duration-300" /></button>
      </div>
    </div>
  );
}
...
export default function ConsultationContent() {
  ...
  const [form, setForm] = useState<FormState>({ treatment: searchParams.get("treatment") ?? "", date: "", slotId: "", selectedSlot: null, patientName: "", patientEmail: "", patientPhone: "", notes: "" });
